...
public class ProAI : AI
{
	...
	{
		var values = GetValues(new FakeBoard(board));

		var column = Library.Max(values);

		if (column < 0 || column >= board.Columns)
		{
			column = new Random().Next(board.Columns);
		}
	...
	protected int[] GetValues(FakeBoard board)
	{
		...
		for (var x = 0; x < board.Columns; x++)
		{
			...
			if (y != 0)
			{
				...
				if (board.CheckVictor(x, y - 1, Team))
				{
					values[x] -= 100;
				}
			}

			if (board.CheckVictor(x, y, Team))
			{
				values[x] += 99999; // take that dub
			}

			var teamValueMultiplier = 1.0;
			var teamCheck = Team;
			for (var i = 0; i < board.TeamCount - 1; i++)
			{
				...
				{
					var otherX = x + (board.Connect - 1);
					var otherY = board.GetNextY(otherX);

					if (y == otherY)
					{
						var found = false;
						for (var j = x + 1; j < otherX; j++)
						{
	...
	private int SmartCheck(int x, int y, State state, FakeBoard board)
	{
		...
		if (y >= board.Connect || board[x, board.Connect - 1] == Team)
		{
			vertical = board.CheckDirection(x, y, 0, 1, state);
		}

		if (x == 0)
		{
			if (board[x + board.Connect - 1, y] == state)
			{
				horizontal = board.CheckDirection(x, y, 1, 0, state) + 2;
			}

			if (y <= board.Rows - board.Connect && board[x + board.Connect - 1, y + board.Connect - 1] == state)
			{
				upDiagonal = board.CheckDirection(x, y, 1, -1, state) + 1;
			}

			if (y >= board.Connect - 1 && board[x + board.Connect - 1, y - board.Connect + 1] == state)
			{
				downDiagonal = board.CheckDirection(x, y, 1, 1, state) + 1;
			}
		}
		else if (x == board.Columns - 1)
		{
			if (board[x - board.Connect + 1, y] == state)
			{
				horizontal = board.CheckDirection(x, y, 1, 0, state);
			}

			if (y <= board.Rows - board.Connect && board[x - board.Connect + 1, y + board.Connect - 1] == state)
			{
				downDiagonal = board.CheckDirection(x, y, 1, 1, state);
			}

			if (y >= board.Connect - 1 && board[x - board.Connect + 1, y - board.Connect + 1] == state)
			{
				upDiagonal = board.CheckDirection(x, y, 1, -1, state);
			}
		}
		else
		{
			return board.CheckAllDirections(x, y, state);
		}
