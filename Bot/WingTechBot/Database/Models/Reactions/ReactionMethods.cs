...
{
	public static async Task<Reaction> Find(ulong giverId, ulong receiverId, ulong channelId, ulong messageId, int emoteId)
	{
		await using BotDbContext context = new();
		return await context.Reactions
			.Include(r => r.Emote)
			.FirstOrDefaultAsync(r => r.GiverId == giverId && r.ReceiverId == receiverId && r.ChannelId == channelId && r.MessageId == messageId && r.EmoteId == emoteId);
	}

	public static async Task<Reaction> AddReaction(ulong giverId, ulong receiverId, ulong channelId, ulong messageId, string emoteName, ulong? discordEmoteId)
	{
		if (giverId == 0 || receiverId == 0 || channelId == 0 || messageId == 0) throw new ArgumentException("Invalid ID");
		...
		if (await Find(giverId, receiverId, channelId, messageId, emote.Id) != null)
			throw new ArgumentException("Reaction exists");

		await using BotDbContext context = new();
		Reaction reaction = new(giverId, receiverId, channelId, messageId, emote.Id);
		await context.Reactions.AddAsync(reaction);
		await context.SaveChangesAsync();
		return reaction;
	}

	public static async Task RemoveReaction(ulong giverId, ulong receiverId, ulong channelId, ulong messageId, string emoteName, ulong? discordEmoteId)
	{
		...
		await using BotDbContext context = new();
		var reaction = await Find(giverId, receiverId, channelId, messageId, emote.Id);
